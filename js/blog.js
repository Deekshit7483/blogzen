    ...
    fetch("../database/jsonData.json")
        ...
        .then(json => {
            json.blogs.map(data => {
                ...
                // Check if the category matches the selected one or if 'All' is clicked
                if (data.id === id || id === 'All') {
                    tbody.innerHTML = "";
...
function buttonClick(category) {
    const urlParams = new URLSearchParams(window.location.search);
    urlParams.set("id", category); // Set the category ID parameter
    window.location.search = urlParams.toString(); // Update the URL with the new parameter
}
